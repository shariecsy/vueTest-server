...
var GetRecordList = function(req, res) {
	...
	pool.query('select id, hospital, department, doctor, result, collect_time from medical_records where user_id=? limit ? offset ?', [userId,pageSize-0,(pageSize*(currentNo-1))-0], function(error, results, fields) {
		if(error) {
			res.json({
				code: -1,
				msg: error
